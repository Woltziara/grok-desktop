...
function treeDepth(root: string, dir: string): number {
  const rel = relativeDisplay(root, dir);
  if (!rel || rel === "." || rel === dir) return 0;
  return rel.split("/").filter(Boolean).length;
}

function changeDepth(path: string): number {
...
export function FileBrowser({
  ...
  const filesDepth =
    project && browseCwd ? treeDepth(project, browseCwd) : 0;
          ...
          {files.map((f) => {
            ...
            return (
              <div
                ...
                style={{ "--tree-depth": filesDepth } as CSSProperties}
                onContextMenu={(e) =>
                  openFileMenu(e, {
                    path: f.path,
                    absPath: f.path,
          ...
          {visibleChanges.map((entry) => {
            ...
            return (
              <div
                ...
                style={
                  { "--tree-depth": changeDepth(entry.path) } as CSSProperties
                }
                onContextMenu={(e) =>
                  openFileMenu(e, {
                    path: entry.path,
