...
## Product direction lock — read before any UI change

**`docs/INTENT-LOCK.md` is frozen.** Home surface is a Claude-like living room for a **non-coder** (cases · conversation · on-demand artifact). It is **not** a Codex/IDE workbench. Colors, fonts, and type follow **Wave Paper & Ink** (one paper `#FAF7F2`, one ink `#57504A`/`#3B3733`, one accent `#0E7490`, 仓耳今楷 + Hack).

Before changing layout, chrome, Welcome, sidebar, composer, or the right panel: read that file. If a change would put file trees, terminals, git, worktrees, diagnostic dumps, or a pill-row toolbar on the home surface, **stop and ask**. Do not start from a blank UI; compose from the sources named in that lock (keep this repo’s ACP shell; restyle after grok-app + Claude artifact timing + Cherry Studio principles; do not copy AGPL code).
...
This GUI (fork): https://github.com/Woltziara/grok-desktop  
Upstream shell: https://github.com/liaan/grok-desktop  
...
5. Verify assets at https://github.com/Woltziara/grok-desktop/releases  
