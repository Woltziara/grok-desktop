 ...
 */
export function sessionForkRequestParams(opts = {}) {
  return {
    sessionId: String(opts.sessionId || "").trim(),
    cwd: String(opts.cwd || ""),
    mcpServers: Array.isArray(opts.mcpServers) ? opts.mcpServers : [],
  };
}
 ...
 * @returns {{ method: string, params: object }[]}
 */
export function sessionForkAttempts(opts) {
  const params = sessionForkRequestParams(opts);
  return [
    { method: "session/fork", params },
    { method: "_x.ai/session/fork", params },
    { method: "x.ai/session/fork", params },
  ];
}

/**
 * Fork must return a **new** session id. Empty or same-as-source is not a fork.
 * @param {any} result
 * @param {string} sourceSessionId
 */
export function parseForkSessionId(result, sourceSessionId) {
  const body =
    result?.result && typeof result.result === "object"
      ? result.result
      : result;
  const raw = String(
    body?.sessionId || body?.newSessionId || "",
  ).trim();
...
/**
 * grok-build rewind: keep the selected user prompt, drop everything after.
 * `targetPromptIndex` is 0-based among user prompts.
 * @param {string} sessionId
 * @param {number} targetPromptIndex
 * @param {boolean} [restoreFiles]
 */
export function rewindExecuteRequestParams(
  sessionId,
  targetPromptIndex,
  restoreFiles = false,
) {
  const sid = String(sessionId || "").trim();
  const index = Math.max(0, Number(targetPromptIndex) || 0);
  const restore = Boolean(restoreFiles);
  return {
    sessionId: sid,
 ...
 * @returns {{ method: string, params: object }[]}
 */
export function rewindExecuteAttempts(
  sessionId,
  targetPromptIndex,
  restoreFiles = false,
) {
  const params = rewindExecuteRequestParams(
    sessionId,
    targetPromptIndex,
    restoreFiles,
  );
  return [
    { method: "_x.ai/rewind/execute", params },
    { method: "x.ai/rewind/execute", params },
  ];
}
