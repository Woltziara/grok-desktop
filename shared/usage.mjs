...
/**
 * Remaining context for a Claude-style meter.
 * `used` is live occupancy (`lastContextTokens`), `windowSize` is the model window.
 * @param {number} used
 * @param {number} windowSize
 */
export function contextMeter(used, windowSize) {
  ...
  if (!win) {
    return {
      ...
      usedPct: null,
      tone: "ok",
      label: usedN > 0 ? `已用 ${formatTokens(usedN)}` : "",
      tooltip:
        usedN > 0
          ? `这一轮已经占用 ${usedN.toLocaleString()} tokens。还不知道这个模型一共能装多少。`
          : "",
    };
  }
  const remaining = Math.max(0, win - usedN);
  const remainingPct = Math.max(0, Math.min(100, Math.round((remaining / win) * 100)));
  const usedPct = Math.max(0, Math.min(100, Math.round((usedN / win) * 100)));
  const tone = remainingPct <= 15 ? "danger" : remainingPct <= 25 ? "warn" : "ok";
  return {
    used: usedN,
    window: win,
    remaining,
    ...
    usedPct,
    tone,
    label: `还剩 ${remainingPct}%`,
    tooltip: [
      `已用 ${formatTokens(usedN)} / 一共 ${formatTokens(win)}`,
      `还剩 ${formatTokens(remaining)}（${remainingPct}%）`,
      remainingPct <= 25
        ? "余量不多了。点一下会先准备胶囊，再压缩，然后强制读 Catch up。"
        : null,
    ]
      .filter(Boolean)
      .join("\n"),
