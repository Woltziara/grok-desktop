...
export function previewPageIdentity(url) {
  if (!/^https?:\/\//i.test(String(url || ""))) return null;
  return crypto.createHash("sha256").update(previewIdentityHref(url) || String(url || "")).digest("hex");
}

function pageId(state) {
  if (!/^[a-f0-9]{64}$/.test(String(state?.pageId || ""))) throw new Error("浏览器页面身份不可用，请重新打开页面再引用。");
  return state.pageId;
}
...
export function validateBrowserReference(reference, state, sessionId) {
  ...
  if (!validShape || !state?.open || state.ownerSessionId !== sessionId ||
      state.leaseId !== reference.leaseId || pageId(state) !== reference.pageId) {
