...
function parkedMap(ws) {
  if (!ws.parkedAgents) ws.parkedAgents = new Map();
  return ws.parkedAgents;
}

/** Session ids whose agent turn is still running (live or parked). */
export function listLiveTurnSessionIds() {
  const ids = [];
  for (const ws of windowSessions.values()) {
    if (ws.agent?.turnOpen && ws.agent.sessionId) {
      ids.push(String(ws.agent.sessionId));
    }
    if (!ws.parkedAgents) continue;
    for (const [id, agent] of ws.parkedAgents) {
      if (agent?.turnOpen) ids.push(String(id));
    }
  }
  return [...new Set(ids)];
}

/** Keep a live agent process when the window focuses another chat. */
function parkLiveAgent(ws) {
  const a = ws.agent;
  if (!a?.sessionId || !a.proc) {
    ws.agent = null;
    return;
  }
  parkedMap(ws).set(String(a.sessionId), a);
  ws.agent = null;
}

function takeParkedAgent(ws, sessionId) {
  if (!sessionId) return null;
  const map = parkedMap(ws);
  const a = map.get(String(sessionId));
  if (!a) return null;
...
export function clearProjectOnWindow(ws) {
  ...
  ws.agentChain = ws.agentChain
    .then(
      () => killAgentClient(a).then(() => killParkedAgents(ws)),
      () => killAgentClient(a).then(() => killParkedAgents(ws)),
    )
    .then(
      () => undefined,
  ...
  const run = async () => {
    ...
    if (agent?.ready && agent.cwd === cwd && agent.proc) {
      ...
      if (forceNew) {
        parkLiveAgent(ws);
        agent = null;
      } else if (resumeSessionId && resumeSessionId !== agent.sessionId) {
        parkLiveAgent(ws);
        agent = null;
      } else if (resumeSessionId && resumeSessionId === agent.sessionId) {
        ...
        return agent;
      } else if (!resumeSessionId && !forceNew) {
        agent.setAllowWritesThisSession(false);
        rememberProjectOnWindow(ws, cwd, agent.sessionId);
        restartBackgroundTaskTail(ws, cwd, agent.sessionId);
        return agent;
    ...
    if (ws.agent) {
      if (ws.agent.sessionId && ws.agent.proc) {
        parkLiveAgent(ws);
        agent = null;
      } else {
        const leftover = ws.agent;
        rememberProjectOnWindow(ws, leftover.cwd, leftover.sessionId);
        clearPendingPermissions(ws);
        await leftover.dispose();
        if (ws.agent === leftover) ws.agent = null;
        agent = null;
        if (!isSessionLive(ws, gen)) {
          throw new Error("Window closed");
        }
    ...
    agent.on(
      ...
      ({ params, respond }) => {
        ...
        const tagged = {
          ...(params || {}),
          sessionId: params?.sessionId || agent.sessionId,
        };
