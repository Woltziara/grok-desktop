...
export function shouldAutoTrustFolder(cwd, workspace) {
  if (isGrokAcpWorktreePath(cwd) || isGrokAcpWorktreePath(workspace)) {
    return true;
  }
  // $HOME / fs root cannot be saved in trusted_folders.toml. Auto-trust so
  // Desktop does not re-ask on every new chat (same as CLI unrecordable-key).
  // Skip empty args: isTooBroadRoot(undefined) is true.
  if (cwd && isTooBroadRoot(cwd)) return true;
  if (workspace && isTooBroadRoot(workspace)) return true;
  return false;
}
